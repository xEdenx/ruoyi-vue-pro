...
public interface BpmTaskConvert {
    ...
    default PageResult<BpmTaskRespVO> buildTodoTaskPage(PageResult<Task> pageResult,
        ...
        return BeanUtils.toBean(pageResult, BpmTaskRespVO.class, taskVO -> {
            ...
            if (CollUtil.isNotEmpty(userMap)) {
                taskVO.getProcessInstance().setStartUser(buildUser(processInstance.getStartUserId(), userMap));
            }
    ...
    default PageResult<BpmTaskRespVO> buildTaskPage(PageResult<HistoricTaskInstance> pageResult,
        ...
        List<BpmTaskRespVO> taskVOList = CollectionUtils.convertList(pageResult.getList(), task -> {
            ...
            // 用户信息
            taskVO.setAssigneeUser(buildUser(task.getAssignee(), userMap));
            // 流程实例
    ...
    default List<BpmTaskRespVO> buildTaskListByParentTaskId(List<Task> taskList,
        ...
        return convertList(taskList, task -> BeanUtils.toBean(task, BpmTaskRespVO.class, taskVO -> {
            taskVO.setAssigneeUser(buildUser(task.getAssignee(), userMap));
            taskVO.setOwnerUser(buildUser(task.getOwner(), userMap));
        }));
    }
    ...
    default BpmMessageSendWhenTaskCreatedReqDTO convert(ProcessInstance processInstance, Task task) {
    ...
    default void buildTaskOwner(BpmTaskRespVO task, String taskOwner,
                                Map<String, BpmPortalOrganizationApi.PortalUser> userMap,
                                Map<String, BpmPortalOrganizationApi.PortalDepartment> deptMap) {
        task.setOwnerUser(buildUser(taskOwner, userMap));
    }
    ...
    default void buildTaskAssignee(BpmTaskRespVO task, String taskAssignee,
                                   Map<String, BpmPortalOrganizationApi.PortalUser> userMap,
                                   Map<String, BpmPortalOrganizationApi.PortalDepartment> deptMap) {
        task.setAssigneeUser(buildUser(taskAssignee, userMap));
    }

    default UserSimpleBaseVO buildUser(String userId, Map<String, BpmPortalOrganizationApi.PortalUser> userMap) {
